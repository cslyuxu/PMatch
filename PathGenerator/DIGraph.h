...
public:
	...
	//For Query Generator
	VertexID QGENgetRandomRoot();
	void QGENInsertEdge(DIGRAPH*);
	VertexID QGENMaximumConnectComponent(int, DIGRAPH *);
	void QGENRemove(VLabels&);
	void QGENOutFile(ofstream *);
...
void DIGRAPH<VLabelType, ELabelType>::initMap() {
	int size = getVcnt();
	this->matrix = new VLabelType[size];
	int i = 0;
	for (typename VLabels::iterator it = getVLabel().begin();
		it != getVLabel().end(); it++) {
		VertexID s = it->first;
	...
	while (!nodes.empty()) {
		...
		for (typename AdjList::iterator it1 = getOutEdge()[v].begin();
			...
			if (query->isLabel(getVLabel(u))) {								//test whether the label of u is in the query
				...
				visit_v.insert(u);
				nodes.push(u);
				//cout<<"Vertex "<<u<<" is in the final set!"<<endl;
			}
		}
...
template<class VLabelType, class ELabelType>
VertexID DIGRAPH<VLabelType, ELabelType>::QGENgetRandomRoot(){
	int point = rand()% this->getVcnt();
    int place = 0;
	VertexID root;
...
template<class VLabelType, class ELabelType>
void DIGRAPH<VLabelType, ELabelType>::QGENInsertEdge(DIGRAPH *graph){
...
template<class VLabelType, class ELabelType>
VertexID DIGRAPH<VLabelType, ELabelType>::QGENMaximumConnectComponent(int Diameter, DIGRAPH *query){
	int MaxNum = 0;
	VertexID place = 0;
    for (auto it = this->getVLabel().begin(); it != this->getVLabel().end(); it ++){
        unordered_set<VertexID> N;
         this->getDNeighbor(it->first, Diameter, N, query);
        if(MaxNum < N.size()){
              MaxNum = N.size();
              place = it->first;
        }
...
template<class VLabelType, class ELabelType>
void DIGRAPH<VLabelType, ELabelType>::QGENOutFile(ofstream *OutFile){
	for (auto it1 = this->getVLabel().begin(); it1 != this->getVLabel().end(); it1 ++){
		*OutFile <<"\r\n#\t";
         *OutFile << it1->first <<"\t"<<it1->second;
        for(auto it2 = this->getOutEdge()[it1->first].begin(); it2 != this->getOutEdge()[it1->first].end(); it2++){
             *OutFile <<"\t"<<it2->first;
         }        
    }
}
...
template<class VLabelType, class ELabelType>
void DIGRAPH<VLabelType, ELabelType>::QGENRemove(VLabels& mark){
	for (auto it = mark.begin(); it != mark.end(); it++){
		if(this->getVLabel().find(it->first)!=this->getVLabel().end())
			this->removeVertex(it->first);        
    }   
}
